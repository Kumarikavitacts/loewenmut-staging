    ...
    <div className="pt_3 bg_theme_work_station" data-aos="zoom-in">
      <div className="work-section">
        ...
        {cards.map((card) => {
          ...
          return (
            <div key={card.number} className={`work-info-card ${card.className}`} style={{ display: isVisible ? "" : "none", }} >
          ...
          <div className="work-phone-content">
            ...
            <div className="work-suggestions">
              {suggestions.map((item, index) => {
                ...
                return (
                  <button key={index} type="button" className={isActive ? "active" : ""} onClick={() => handleSuggestionClick(item) } style={{ backgroundColor: isActive ? "var(--bs-themecolor)" : undefined, }}> {item} </button>
                );
              })}
            </div>
            ...
            <form className="work-search" onSubmit={handleSubmit}>
              <input type="text" value={message} onChange={handleInputChange} readOnly={true} placeholder="Gib eine Nachricht ein..."/>

              {message && (
                 <span className="cursorPointer d-flex justify-content-center " onClick={handleClear}>
